...
export function MultiRepoApp({ initialCwd }: { initialCwd: string }) {
  ...
  const handleAddRepository = useCallback(async (path: string) => {
    try {
  ...
  const handleCloseRepository = useCallback(async (repositoryId: string) => {
  ...
  return (
    <box width="100%" height="100%" flexDirection="column">
      ...
      {activeRepository && (
        <App key={activeRepository.id} cwd={activeRepository.path} />
      )}
